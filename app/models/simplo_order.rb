...
class SimploOrder < ApplicationRecord
  def self.integrate_order_items(initial_date)
    initial_date = "?data_inicio=#{initial_date.to_date.strftime('%Y-%m-%d')}" if initial_date.present?
    @order_page = HTTParty.get("https://purchasestore.com.br/ws/wspedidos.json#{initial_date}",
